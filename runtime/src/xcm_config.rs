...
parameter_types! {
	...
	pub const ThisNetwork: NetworkId = CustomNetworkId::Evochain.as_network_id();
	/// The Rialto network ID.
	pub const RialtoNetwork: NetworkId = CustomNetworkId::Rococo.as_network_id();
	/// The OwnershipParachain network ID.
	pub const OwnershipParachainNetwork: NetworkId = CustomNetworkId::OwnershipParachain.as_network_id();
...
pub type OnEvochainBlobDispatcher = xcm_builder::BridgeBlobDispatcher<
...
impl xcm_executor::Config for XcmConfig {
	...
	type FeeManager = ();
	type MessageExporter = ToRialtoOrOwnershipParachainSwitchExporter;
...
pub struct ToRialtoOrOwnershipParachainSwitchExporter;

impl ExportXcm for ToRialtoOrOwnershipParachainSwitchExporter {
	...
	fn validate(
		...
		if network == RialtoNetwork::get() {
		...
		} else if network == OwnershipParachainNetwork::get() {
			ToOwnershipParachainBlobExporter::validate(
				network,
				channel,
				universal_source,
				destination,
			...
			.map(|result| ((OwnershipParachainNetwork::get(), result.0), result.1))
		} else {
	...
	fn deliver(ticket: Self::Ticket) -> Result<XcmHash, SendError> {
		...
		if network == RialtoNetwork::get() {
			...
			ToOwnershipParachainBlobExporter::deliver(ticket)
		} else {
			Err(SendError::Unroutable)
		}
	}
...
/// Emulating XCMP channel with sibling chain. We don't have required infra here, at Evochain,
...
mod tests {
	use super::*;
	use crate::{
		rialto_messages::FromRialtoMessageDispatch, WithOwnershipParachainMessagesInstance,
	...
	fn xcm_messages_to_rialto_parachain_are_sent_using_bridge_exporter() {
		new_test_ext().execute_with(|| {
			// ensure that the there are no messages queued
			let lane_id =
				crate::ownership_parachain_messages::ToOwnershipParachainXcmBlobHauler::xcm_lane();
			OutboundLanes::<Runtime, WithOwnershipParachainMessagesInstance>::insert(
				lane_id,
				OutboundLaneData::opened(),
			);
			assert_eq!(
				OutboundLanes::<Runtime, WithOwnershipParachainMessagesInstance>::get(lane_id)
					.unwrap()
					.latest_generated_nonce,
				0,
			);
			...
			XcmExecutor::<XcmConfig>::execute_xcm_in_credit(
				Here,
				prepare_outbound_xcm_message(OwnershipParachainNetwork::get()),
				Default::default(),
				Weight::MAX,
				Weight::MAX,
			)
